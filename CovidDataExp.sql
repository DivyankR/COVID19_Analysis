...
from PortfolioProject..CovidDeaths
where continent!='Null' and location like '%India%'
order by 1,2
...
select dea.continent, dea.date , dea.location, vac.new_vaccinations,dea.population,sum(vac.new_vaccinations) over (partition by dea.location order by dea.location, dea.date) as RollingPeopleVaccinated
...
with PopvsVac(continent,date,location,new_vaccinations,population,RollingPeopleVaccinated)
as
(
select dea.continent, dea.date , dea.location, vac.new_vaccinations,dea.population,sum(vac.new_vaccinations) over 
...
select *,(convert(float,RollingPeopleVaccinated)/convert(float,nullif(population,0)))*100 as VaccinationPercentage
from PopvsVac

-- Creating a view for finding percentage of people vaccinated
